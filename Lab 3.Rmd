...
```{r}
my_sample <- rnorm(n=15,mean=17,sd=2)
hist(my_sample)
```
```{r}
...
```{r}
#create different samples with different descriptives
observations <- rnorm(n=5*25, mean=200, sd=100)
samples <- rep(1:5,each =25)
my_data <- data.frame(samples,observations)

#get means of samples taken
means <- my_data %>%
  group_by(samples) %>%
  summarize(sample_mean = mean(observations),
            sample_sd = sd(observations))

#plot means by sample
ggplot(means,aes(x = samples, y = sample_mean))+
  geom_bar(stat="identity") +
...
summarized_data <- my_data %>%
  group_by(samples) %>%
  summarize(samples_means = mean(observations))
...
#making a for loop to test for each different sample size
...
Both standard deviations formulas using N or N-1 are biased especially in the smaller sample sizes #2,5,10 and remain below the actual standard deviation of the population. Although, N-1 is less biased in finding the standard deviation closest to the population sd.